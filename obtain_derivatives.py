...
def main(sim_name, z_nbody, R_smooth, machine, par_vary, plot_derivatives=False):
    ...
    for i, key in enumerate(Pk_templates.keys()):
        ...
        dPk_templates[key+','+par_vary] = dPk_tmp
        ...
        if plot_derivatives:
            ...
            plt.ylim([0.8, 1.2])
            plt.savefig("figs/deriv_"+par_vary+"_"+key2str(key)+"_z%4.3f.png"%z_nbody)
            plt.close()

    print(dPk_templates.keys(), dPk_templates.values())
    # save as asdf file
    save_asdf(dPk_templates,"dPk_templates_%d.asdf"%(int(R_smooth)),data_dir)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--par_vary', help='Parameter we are testing', default=DEFAULTS['par_vary'])
    parser.add_argument('--plot_derivatives', help='Plot derivatives to check convergence', action='store_true')
